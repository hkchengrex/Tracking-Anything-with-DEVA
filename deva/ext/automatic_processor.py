...
def make_segmentation(cfg: Dict, image_np: np.ndarray, sam_model: SamAutomaticMaskGenerator,
                      min_side: int, suppress_small_mask: bool) -> (torch.Tensor, List[ObjectInfo]):
    mask, segments_info = auto_segment(cfg, sam_model, image_np, min_side, suppress_small_mask)
...
def process_frame_automatic(deva: DEVAInferenceCore,
    ...
    if cfg['temporal_setting'] == 'semionline':
        if ti + cfg['num_voting_frames'] > deva.next_voting_frame:
            mask, segments_info = make_segmentation(cfg, image_np, sam_model, new_min_side,
                                                    suppress_small_mask)
