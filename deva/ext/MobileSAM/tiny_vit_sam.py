# https://github.com/IDEA-Research/Grounded-Segment-Anything/blob/main/EfficientSAM/MobileSAM/tiny_vit_sam.py
...
class MBConv(nn.Module):
    def __init__(self, in_chans, out_chans, expand_ratio, activation, drop_path):
        ...
        self.conv2 = Conv2d_BN(self.hidden_chans,
                               self.hidden_chans,
                               ks=3,
                               stride=1,
                               pad=1,
...
class Attention(torch.nn.Module):
    ...
    def forward(self, x):  # x (B,N,C)
        ...
        qkv = self.qkv(x)
        # (B, N, num_heads, d)
        q, k, v = qkv.view(B, N, self.num_heads, -1).split([self.key_dim, self.key_dim, self.d],
                                                           dim=3)
        # (B, num_heads, N, d)
        q = q.permute(0, 2, 1, 3)
        k = k.permute(0, 2, 1, 3)
    ...
    def forward(self, x):
        ...
        if H == self.window_size and W == self.window_size:
            ...
            x = x.view(B, nH, self.window_size, nW, self.window_size,
                       C).transpose(2, 3).reshape(B * nH * nW, self.window_size * self.window_size,
                                                  C)
            x = self.attn(x)
            # window reverse
